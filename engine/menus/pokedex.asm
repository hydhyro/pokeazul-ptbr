...
; draw the horizontal line separating the seen and owned amounts from the menu
						ld hl, hUILayoutFlags	;recupera o caimento de linha para o padrão 1 --  POR ALGUM MOTIVO SEM ISSO BUGA a quebra de linha
					res 2, [hl]				;recupera o caimento de linha para o padrão 1 -- até outro comando carregar text_box.asm de novo

	hlcoord 14, 8
	ld a, "─"
	ld [hli], a ;cada tile da linha
	ld [hli], a
	ld [hli], a
	ld [hli], a
	ld [hli], a
	ld [hli], a
				;hlcoord 6, 0 ; posição do acento do ú
				;;ld [hl], $e9 ; aciona acento ´ do CONTEÚDO
				;hlcoord 15, 13 ; posição do acento do área
				;;ld [hl], $e9 ; aciona acento ´ do ÁREA
	hlcoord 13, 0
...
.loop
	...
	ldh [hAutoBGTransferEnabled], a
	hlcoord 3, 2	;começo
	lb bc, 14, 10			;limpa a parte dos nomes
	call ClearScreenArea
	hlcoord 1, 3				;posição da lista de nomes + números. normal 1, 3
	ld a, [wListScrollOffset]
	ld [wd11e], a
	ld d, 7
	ld a, [wDexMaxSeenMon]
...
.printPokemonLoop
	...
	ld de, SCREEN_WIDTH
	add hl, de
	dec hl ;cada linha dessa muda a lista de nomes 1 tile pra esquerda
	dec hl ;muda a lista de nomes 1 tile pra esquerda
	push hl
	ld hl, wPokedexOwned
	call IsPokemonBitSet
	pop hl
...
LoadAlturaPesogfx:; Carrega o gráfico TREINADOR para os ingame Trade Trainer's name
    ld de, AlturaPesogfx ; the graphic data
    ld hl, vChars1 tile $3B ; where in vram it will be placed
    lb bc, BANK(AlturaPesogfx), 5 ; number of tiles the graphic takes up horizontally
    jp CopyVideoData