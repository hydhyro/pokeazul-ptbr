...
SlidePlayerAndEnemySilhouettesOnScreen:
	...
	ld a, MESSAGE_BOX ; the usual text box at the bottom of the screenç
	ld [wTextBoxID], a
...
.next3
	...
	jp nz, .next4					;;changed from jr
	
	ld a, [wEnemyMonSpecies2]		;;hydalt == checagem de qual pokémon para imprimir genero correto
	;ld [wcf91], a					;; ; load a into [wcf91] only need to happen once, the pokemon specie doesnt change after every check, a check is just a check
	cp 	NIDORAN_F					;;	; check if the value in A match the value behind the pokemon here
	jr z, .ifFemale					;;	; keep the jr z, if NIDORAN_F = A then the cp result in 0, it set the Z flag (z as in ZERO), so "if Z flag set, jr to .ifFemale"
	cp NIDORINA						;;	
	jr z,  .ifFemale				;;
	cp NIDOQUEEN					;;	
	jr z,  .ifFemale				;;	by Engezerstorung
	cp CHANSEY						;;
	jr z,  .ifFemale				;;	
	cp KANGASKHAN					;;	
	jr z,  .ifFemale				;;	
	cp JYNX							;;	
	jr z, .ifFemale					;;								
	ld hl, TrainerAboutToUseText	;;normal text / male ; loading the male text
	jr .contnext3					;;skip the female; if you have reached here, all the check for female failed and therefore not female, male text have been loaded above, and now jr to .contnext3
.ifFemale							;;do the female text
	ld hl, TrainerAboutToUseFemaleText	 ;;jr'd here because female, loading female text
.contnext3	
	call PrintText					;;hydalt  alterado para fazer checagem de genero
	hlcoord 0, 7
	lb bc, 8, 1
	ld a, TWO_OPTION_MENU
	ld [wTextBoxID], a
...
TrainerAboutToUseFemaleText: ;;hydalt genero feminino
	text_far _TrainerAboutToUseFemaleText
	text_end
...
DrawPlayerHUDAndHPBar:
	...
	call PrintLevelFull ;hydalt imprime lv100 com o :N / :L (pokémon aliado)
.doNotPrintLevel
...
DrawEnemyHUDAndHPBar:
	...
	callfar PlaceEnemyHUDTiles
		;==============================	Código para mostrar marcação de Pokémon Capturado
	push hl
	ld a, [wEnemyMonSpecies2]
	ld [wd11e], a
	...
	and a
	;coord hl, 1, 2
	;ld [hl], $e7;icone de não capturado !$73 linha normal
	jr z, .notOwned
	coord hl, 1, 1
	ld [hl], $72;icone da pokébola de capturado !
.notOwned
	pop hl
	;==============================
	ld de, wEnemyMonNick
	...
	call PrintLevelFull 	;hydalt imprime lv100 com o :N / :L (pokémon oponente) 
.skipPrintLevel
	ld hl, wEnemyMonHP
	ld a, [hli]
	ldh [hMultiplicand + 1], a
...
; put cursor in left column for normal battle menu (i.e. when it's not a Safari battle)
	...
	ld b, $3 ; top menu item X;batalhar
...
; put cursor in right column for normal battle menu (i.e. when it's not a Safari battle)
	ldcoord_a 3, 14 ; clear upper cursor position in left column
	ldcoord_a 3, 16 ; clear lower cursor position in left column
...
; item menu was selected
	;inc a ; increment a to 2 ;;anuleir para voltar ao padrão japones
	jr .handleMenuSelection
.notItemMenu
	cp $2 ; was the party menu selected?
	jr nz, .handleMenuSelection
; party menu selected
	;dec a ; decrement a to 1 ;;anuleir para voltar ao padrão japones
.handleMenuSelection
	and a
...
.quitPartyMenu
	...
	jp DisplayBattleMenu
.partyMonDeselected
	hlcoord 10, 11;cobrir o menu de troca/sumário/cancelar
...
.partyMonWasSelected
	...
	ld [wTextBoxID], a
	call DisplayTextBoxID
				;hlcoord 15, 13 ; posição do acento do á - sumário
				;;ld [hl], $e9
	ld hl, wTopMenuItemY
	ld a, $c
	ld [hli], a ; wTopMenuItemY
...
.loadmoves
	ld de, wMoves
	ld bc, NUM_MOVES
	;call CopyData							;usar nomes de golpe padrão
	;callfar FormatMovesString				;;usar nomes de golpe padrão
	call CopyData							;alternativo
    ld a, 1									;alternativo
    ld [wUseAlternateMoveNames], a			;alternativo
    callfar FormatMovesString				;alternativo
    xor a									;alternativo
    ld [wUseAlternateMoveNames], a			;alternativohydalt
	ret
...
SelectMenuItem:
	...
	dec a
	jr nz, .select
				hlcoord 2, 15 ; posição do acento do ú / hydalt
				ld [hl], " " ; apaga o acento do Mímica
	hlcoord 1, 14
...
.battleselect
	...
	and a
	jr z, .select
	hlcoord 1, 13;hyd alt posição da seta de troca de golpes
	dec a
	ld bc, SCREEN_WIDTH
	call AddNTimes
	ld [hl], "▷"
...
WhichTechniqueString:
	db "Qual té´cnica quer"
	next "copiar com MI´´MICA?@"
