...
.next
	...
	jr nz, .metBill
		;hlcoord 12, 1 ; posição do acento do É
		;ld [hl], $e9 ; aciona acento é ALGUÈM
	hlcoord 2, 2
...
SomeonesPCText:   db "PC de ALGUE´´M@"
BillsPCText:      db "PC do BILL@"
PlayersPCText:    db "PC de @"
OaksPCText:       db "PC do PROF.<PKMN>@"
PKMNLeaguePCText: db "LIGA #MON@"
LogOffPCText:     db "DESLIGAR@"
...
BillsPCMenu:
	...
	ld hl, WhatText
	call PrintText
	hlcoord 11, 13;posição da caixa box
	ld b, 2;tamanho da caixa box
	ld c, 7
	call TextBoxBorder
	ld a, [wCurrentBoxNum]
	and $7f
...
.boxNotEmpty
						hlcoord 1, 1				;posição 
						ld de, RETIRARTEXT			;
						call PlaceString			;chamada do texto; withdraw
	ld a, [wPartyCount]
	cp PARTY_LENGTH
	jr nz, .partyNotFull
						hlcoord 1, 1				;posição 
...
.next
	hlcoord 10, 12
	call PlaceString
		;hlcoord 13, 13 ; posição do acento do á
		;ld [hl], $e9
	hlcoord 10, 14
...
DepositPCText:  db "DEPOSITAR@"
WithdrawPCText: db "RETIRAR@"
StatsCancelPCText:
	db   "SUMA´´RIO"
	next "CANCELAR@"
