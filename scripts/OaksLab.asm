...
OaksLabOakGivesPokedexScript:
	...
	call DisplayTextID
	call DelayFrame
;hydalt só para exibir um texto diferente se o nome for VERMELHO	
	ld hl, wPlayerName
  ld b, -1
.loop
  inc b
  ...
  jr nz, .loop
	...
	cp 8
	jr nz, .normalgamered
	ld a, TEXT_OAKSLAB_OAK_GOT_POKEDEXRED
	jr .normalcode4
	
.normalgamered	
	ld a, TEXT_OAKSLAB_OAK_GOT_POKEDEX
.normalcode4	;hydalt só para exibir um texto diferente se o nome for VERMELHO
	ldh [hSpriteIndexOrTextID], a
	call DisplayTextID
	call Delay3
	ld a, HS_POKEDEX_1
...
OaksLab_TextPointers:
	...
	dw_const OaksLabOakGotPokedexTextRED,         TEXT_OAKSLAB_OAK_GOT_POKEDEXRED
...
OaksLabOakGotPokedexTextRED:
	text_far _OaksLabOakGotPokedexTextRED
	sound_get_key_item
	text_end	
